...
class VisionProOmni:
    ...
    def _hud(self, frame):
        ...
        if VOICE_AVAILABLE:
            mic_col = C_GREEN if self._mic_status=="LISTENING" else C_GOLD
            self._glass(frame,14,H-38,220,28,0.6,mic_col)
            cv2.putText(frame,f"MIC: {self._mic_status}",(22,H-18),FONT,0.38,mic_col,1)
    ...
    def _voice_listen(self):
        ...
        try:
            ...
            SILENCE_DB = 500          # RMS threshold — below = silence
            SPEECH_SEC = 3.5          # max recording window in seconds
            SILENCE_SEC= 0.7          # silence after speech triggers recognition
                ...
                while True:
                    ...
                    if rms > SILENCE_DB:
                        ...
                        if silence_frames >= SILENCE_CHUNKS or len(buf) >= MAX_BYTES:
                            # End of utterance — send to Google
                            audio = sr.AudioData(buf, RATE, 2)
                            buf = b""; recording = False; silence_frames = 0
                            try:
                                text = recognizer.recognize_google(audio).lower()
                                # Accumulate into a rolling 2-phrase window so
                                # "jarvis" and "hide face" as separate utterances still match
                                with self._voice_lock:
                                    prev = self._voice_cmd
                                    # keep last phrase + new phrase joined
                                    joined = (prev + " " + text).strip()[-120:]
                                    self._voice_cmd = joined
